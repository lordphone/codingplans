...
└── web/                   # Angular app (project name: coding-plans)
    ...
    └── src/
        ...
        │   ├── types/
        │   │   ├── database.types.ts        # auto-generated by `npm run db:types` (DB row shapes only)
        │   │   └── database.shapes.ts       # hand-written PostgREST embed shapes
...
**Source of truth:** live Postgres in Supabase. Migrations live in `supabase/migrations/`; apply via `supabase db push`. Regenerate `database.types.ts` after schema changes:

```bash
cd web && npm run db:types   # → supabase gen types typescript --linked
```

That script overwrites only `database.types.ts` (DB row shapes); hand-written embed shapes stay in `database.shapes.ts`.
...
- **State:** `signals` + `computed` view model; search filters rows client-side (provider name/slug, plan name/slug, model name, quantization status word, usage label).
- **Quantization UI:** `QuantizationStatus` = `aggressive` | `standard` | `untested` (see `directory.models.ts`). Maps directly from `benchmark_runs.aggressively_quantized` (`true`→aggressive, `false`→standard, `null`/no rows→untested) — no string-label inference. Rendered as colored dot + word in directory, plan, and provider pages.
